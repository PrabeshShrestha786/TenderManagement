...
public class TenderDaoImpl implements TenderDao {
    ...
    public List<TenderStatusBean> getAllAssignedTenders() {
        ...
        try {
            String query = "SELECT ts.tid, t.tname, ts.vid, v.vname, ts.bid, ts.status "
                         + "FROM tenderstatus ts "
                         + "JOIN tender t ON ts.tid = t.tid "
                         + "JOIN vendor v ON ts.vid = v.vid";

            ps = con.prepareStatement(query);
            rs = ps.executeQuery();

            while (rs.next()) {
                TenderStatusBean status = new TenderStatusBean();
                status.setTendorId(rs.getString("tid"));
                status.setTenderName(rs.getString("tname"));
                status.setVendorId(rs.getString("vid"));
                status.setVendorName(rs.getString("vname"));
                status.setBidderId(rs.getString("bid"));
